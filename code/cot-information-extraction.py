...
def create_prompt(example_timeline,question1,task1_answer1,task2_answer1,question2,task1_answer2,task2_answer2,question3,task1_answer3,task2_answer3,timeline,question):
...
Premise: {timeline}
Question: {question}
Provide answers for task 1,and task 2 separately. Also, give a final answer based on the reasoning in task 2. 
Task 1 Answer:
Task 2 Answer:
Final Answer:
"""
...
for data_folder in data_folders:
    for prediction_folder,model in zip(prediction_folders,models):
        ...
        with open(f'data/{data_folder}/multi-stage-cot-examples/key_retrieval/few_shot_example.json','r') as f:
            few_shot_timeline = json.load(f)
        with open(f'data/{data_folder}/multi-stage-cot-examples/key_retrieval/COT_questions.json','r') as f:
        ...
        for entity in entities:
            ...
            for questions_category in entities[entity]:
                for question in entities[entity][questions_category]:
                    ...
                    prompt = create_prompt(few_shot_timeline,cot_questions['Q1'],cot_questions['A1-Task1'],
                                           cot_questions['A1-Task2'],cot_questions['Q2'],
                                           cot_questions['A2-Task1'],cot_questions['A2-Task2'],
                                           cot_questions['Q3'],cot_questions['A3-Task1'],
                                           cot_questions['A3-Task2'],timeline,actual_question)
                    task = {
                    ...
                    tasks.append(task)
        file_name = f"data/{data_folder}/new_predictions/multi-stage/cot/two-stage(key)/{prediction_folder}/batch-questions-new-gpt-4o.jsonl"
        ...
        if batch_job_status.status == "completed":
            ...
            result_file_name = f"data/{data_folder}/new_predictions/multi-stage/cot/two-stage(key)/{prediction_folder}/batch-results.jsonl"
